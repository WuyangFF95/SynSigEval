...
#' Assess/evaluate multiple summarized runs for one dataset from one software approach.
#'
#' Summarize results from each software approach in \code{tool.dir}/\code{run.names}
#' (generated by running a software approach),
...
#'
#' @param toolName Name of software approach. (e.g. "sigproextractor")
#'
...
SummarizeMultiRuns <-
  function(datasetName,
    ...
    multiRun <- list()
    ## Save name of the software approach and the dataset.
...
#'
#' Summarize results from each software approach in \code{third.level.dir}/\code{tool.dirnames}
...
#'
#' @param toolNames Names of software approach. (e.g. "sigproextractor")
#'
  ...
  function(dataset.dirs,
    ...
    ## and software approaches for easier plotting.
    {
      ...
      for(datasetDir in dataset.dirs){
        ...
        ## Calculate values for each tool/software approach.
        toolNames <- unique(multiTools$averCosSim$toolName)
    ...
    ## in all runs and in all datasets.
    {
      ...
      ## Plot a multi-facet ggplot for all measures and all runs.
      {
        ...
        ggplotList$general <- ggplot2::ggplot(
          ...
          ## Change axis titles
          ggplot2::labs(x = "Software approach") +
      ...
      ## Plot a multi-facet ggplot for all gtSigNames and all runs.
      {
        ...
        ggplotList$general <- ggplotList$general +
          ...
          ggplot2::ggtitle(label = "Average cosine similarity between ground-truth and extracted signatures",
                           subtitle = "for all software approaches, ratios and correlation values.") +
          ## Change axis titles
          ggplot2::labs(x = "Software approach",
                        y = "Cosine Similarity") +
          ## Rotate the axis.text.x (names of tools),
...
#' as the label of the \code{datasetSubGroup} Pearson's R^2.
#'
#' @param toolName Name of software approach to be investigated
#' (e.g. "sigproextractor")
#'
